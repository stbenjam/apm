...
class TestGetAllDependenciesPerf:
    ...
    def test_sort_latency(self, dep_count: int):
        ...
        assert elapsed < 0.5, (
            f"Sorting {dep_count} deps took {elapsed:.3f}s (limit 0.5s)"
        )
...
class TestSemanticEquivalencePerf:
    ...
    def test_identical_lockfiles(self, dep_count: int):
        ...
        assert elapsed < limit, (
            f"Comparing {dep_count} deps took {elapsed:.3f}s (limit {limit}s)"
        )
...
class TestToYamlPerf:
    ...
    def test_to_yaml_latency(self, dep_count: int):
        ...
        lf.local_deployed_file_hashes = {
            f"local-{i}.md": f"sha256:{'cd' * 32}" for i in range(20)
        }
...
class TestGetAllDependenciesCacheOpportunity:
    ...
    def test_repeated_calls(self):
        ...
        assert elapsed < 1.0, (
            f"{call_count} calls took {elapsed:.3f}s total "
            f"({elapsed / call_count:.4f}s per call)"
        )
...
class TestSemanticEquivalenceWithDiff:
    ...
    def test_key_set_mismatch(self):
        ...
        assert elapsed_full < 0.5, (
            f"Full scan took {elapsed_full:.3f}s (limit 0.5s)"
        )
    ...
    def test_last_dep_value_diff(self):
        ...
        lf2.dependencies[last_key].deployed_files.append(
        ...
        assert result is False
        assert elapsed < 1.0, (
            f"Full scan with last-dep diff took {elapsed:.3f}s (limit 1.0s)"
        )
