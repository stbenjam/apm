...
class TestConvertToGeminiRules:
    """Test the _convert_to_gemini_rules() frontmatter conversion helper."""

    def test_strips_apply_to_frontmatter(self):
        content = "---\napplyTo: 'src/**/*.py'\n---\n\n# Python rules"
        result = InstructionIntegrator._convert_to_gemini_rules(content)
        assert "applyTo" not in result
        assert "---" not in result
        assert "# Python rules" in result
    ...
    def test_strips_description_frontmatter(self):
        content = "---\ndescription: General rules\napplyTo: '**/*.py'\n---\n\n# Rules"
        result = InstructionIntegrator._convert_to_gemini_rules(content)
        assert "description:" not in result
        assert "applyTo" not in result
...
class TestGeminiRulesIntegration:
    """Test integrate_instructions_for_target() with gemini target."""
    ...
    def test_skips_when_no_gemini_dir(self):
        """Returns empty result when .gemini/ doesn't exist."""
        pkg = self.project_root / "package"
        inst_dir = pkg / ".apm" / "instructions"
        inst_dir.mkdir(parents=True)
        (inst_dir / "python.instructions.md").write_text("# Python")
        ...
        result = self.integrator.integrate_instructions_for_target(
            KNOWN_TARGETS["gemini"], pkg_info, self.project_root
        )
        assert result.files_integrated == 0

    def test_deploys_when_gemini_dir_exists(self):
        """Deploys .md files when .gemini/ exists."""
        ...
        assert result.files_integrated == 1
        deployed = self.project_root / ".gemini" / "rules" / "python.md"
    ...
    def test_renames_instructions_md_to_md(self):
        """File extension is changed from .instructions.md to .md."""
        (self.project_root / ".gemini").mkdir()

        pkg = self.project_root / "package"
        inst_dir = pkg / ".apm" / "instructions"
        inst_dir.mkdir(parents=True)
        (inst_dir / "coding-style.instructions.md").write_text("# Coding style")
        ...
        assert result.files_integrated == 1
        assert (self.project_root / ".gemini" / "rules" / "coding-style.md").exists()
        assert not (self.project_root / ".gemini" / "rules" / "coding-style.instructions.md").exists()
